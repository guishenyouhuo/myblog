...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        ...
        $('#id_table_blog').bootstrapTable({
            url: getRootPath() + '/user/blog_list.j',          //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
function deleteArticleInBulk() {
    ...
    $.ajax({
        type: "DELETE",
        url: getRootPath() + "/user/blog_delete.j",
 ...
 */
function addBlog() {
    c_location(getRootPath() + "/user/blogadd");
}
...
function modifyBlog() {
    ...
    if (dataSel.length > 1) {
        ...
        id = dataSel[0].id;
        c_location(getRootPath() + "/user/blogmodify/" + id);
    }
}